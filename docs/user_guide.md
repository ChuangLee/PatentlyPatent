...
**没有「续作」按钮**：项目挖到一半被打断不需要专门按钮。回到项目时，顶部会显示「上次进度 N/M · 当前：xx」状态提示；你在对话框直接打字（哪怕只是「继续」或对 AI 上次提问的回答）发送 —— AI 会先 `read_user_file('项目计划.md')` 拿回上次的工作状态再续上下文，比专门「续作」按钮更自然，也不会丢失你刚回答的内容。
...
**「项目计划.md」是 AI 的工作台账**：开工时声明步骤；过程中每完成一步更新状态 + 写一句小结 + 关联产出文件；最后给出决策动作 + 关键产出。员工随时打开它就知道 AI 在做什么、做到哪、出了什么文件。系统重启 / 你关浏览器后再回来 → AI 每轮开工前必读这个文件，自动从上次进度接着干，**你只需在对话框继续聊**。
...
| 自动挖掘没启动 | 自动启**只在「新建」入口生效**；再次打开已有项目要么点 ▶ 开始挖掘（全新项目）要么在对话框直接发消息让 AI 续上（有进度的项目） |
| 关浏览器后回来想接着干 | 直接在对话框输入消息（AI 上次的提问 / 「继续」），AI 会自动读项目计划.md 接上下文 |

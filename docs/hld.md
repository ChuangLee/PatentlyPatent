...
**为什么没有续作按钮**：如果上次中断时 AI 正在等申请人回答，专门的「续作」按钮会让用户的回答从对话历史里被"绕开"——按钮逻辑塞 "从待办区第一个未完成 step 续作" 到 prompt 头，AI 收不到回答 → 重新追问或瞎续。**对话原生方案天然避免**：用户在对话框输入消息（不管是回答 AI 的提问还是简单"继续"），都走正常 `interview_stream`，对话历史完整传给 agent。

工作机制：SYSTEM_PROMPT 铁律要求 agent 每轮开工先 `read_user_file('项目计划.md')` 拿上次进度，再消化用户最新消息推进下一步。员工不需要关心"是否要点续作按钮"——直接说话就行。
